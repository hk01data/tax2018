...
export default {
  ...
  computed: {
    STCOut1 () {
      ...
      // 入息
      this.slfNCI = parseFloat(this.slfIncome)
      this.spsNCI = parseFloat(this.spsIncome)
      if (this.slfNCI > 0) {
      ...
      this.STCIn9 = (parseFloat(this.spouse_disabled_dependent_DIS) === 1)
      ...
      if (this.slfOE + this.slfSEE > this.STCIn2) {
        this.slfOE = this.STCIn2 - this.slfSEE
      }
      if (this.spsOE + this.spsSEE > this.STCIn3) {
        this.spsOE = this.STCIn3 - this.spsSEE
      }
  ...
  methods: {
    ...
    get_rate () {
      ...
      if (this.AssessYear === '2017-2018') {
        ...
        this.DPA = 46000 // PDPAAMT
        this.ADPA = 46000 // PADPAAMT
        this.DIS_DA = 75000 // PDDA
        this.SDPGPA = 23000 // PSDPGPA
        this.SADPGPA = 23000 // PSADPGPA
        this.VAPRP_RATE = 10
        this.TAX_RANGE[0] = 0
        this.TAX_RANGE[1] = 45000
        this.TAX_RANGE[2] = 45000
        this.TAX_RANGE[3] = 45000
        this.TAX_RANGE[4] = 0
        this.TAX_RANGE[5] = 0
        this.TAX_RANGE[6] = 0
        this.TAX_RANGE[7] = 0
    ...
    get_deduction () {
      var YrValue = '2017-2018'
      // parent.LSPYrEnd = YrValue
      // // //
      if (YrValue === '2017-2018') {
    ...
    T1OnBlur () {
      ...
      if (Income === '0') {
        vm.spouse_disabled_dependent_DIS = '' + 0
        vm.slfResi = '0'
      }
      vm.slfIncome = Income
      if (Income !== '') {
    ...
    ChkDD (X) {
      ...
      vc = (vm.spsLoan !== vm.oT12)
      if ((X === 12 && vc) || NotNIL(vm.spsLoan)) {
        if (IsNIL(vm.spsIncome)) {
          uv = 0
        } else {
        ...
        if (vc) vm.T12tag = '0'
        if (NotNIL(vm.spsLoan)) {
          iv = vm.FormatInput(vm.spsLoan, 0, uv)
          a = vm.FormatInput(vm.spsLoan, 0, 99999999999)
          v1 = '0'
          if (vm.YrEnd < 2001 && iv !== '0') {
            MsgID = 3
      ...
      if (X > 0) {
        ...
        else if (X === 12) NullZero(vm.spsLoan, MsgID)
