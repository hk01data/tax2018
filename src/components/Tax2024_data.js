  ...
  methods: {
    ...
    ChkDD (X) {
      ...
      if (IsNIL(vm.tax.slfIncome)) {
        if (NotNIL(vm.tax.slfDona) || NotNIL(vm.tax.slfSEE) || NotNIL(vm.tax.slfERCE) || NotNIL(vm.tax.slfMpf) || vm.tax.slfElder > 0 || vm.tax.slfDisdep > 0 || NotNIL(vm.tax.slfOE) || NotNIL(vm.tax.slfRent) || NotNIL(vm.tax.slfBook)) {
          ClrTxt(vm.tax.slfDona)
          console.log('vm.tax.slfDona')
      ...
      if (IsNIL(vm.tax.spsIncome)) {
        if (NotNIL(vm.tax.spsDona) || NotNIL(vm.tax.spsSEE) || NotNIL(vm.tax.spsERCE) || NotNIL(vm.tax.spsLoan) || vm.tax.spsElder > 0 || vm.tax.spsDisdep > 0 || NotNIL(vm.tax.spsOE) || NotNIL(vm.tax.spsBook)) {
          ...
          ClrTxt(vm.tax.spsRent)
          // 2023mock
          ClrTxt(vm.tax.spsBook)
      ...
      // 2023mock
      if (NotNIL(vm.tax.slfBook)) {
        iv = vm.FormatInput(vm.tax.slfBook, 0, vm.LimD_BOOK)
        a = vm.FormatInput(vm.tax.slfBook, 0, 99999999999)
        v1 = '0'
        if (vm.YrEnd < 2023 && iv !== '0') {
          MsgID = 3
          b = 2023
        } else if (iv === '*') {
          MsgID = 1
          ...
          MsgID = 7
          b = a
          v1 = FormatMoney(vm.LimD_BOOK)
        } else {
          v1 = iv
        }
        // console.log('slfSEE', vm.tax.slfSEE, v1, iv, vm.LimD_Education)
        vm.tax.slfBook = SetTxt(vm.tax.slfBook, v1)
      }
      if (NotNIL(vm.tax.spsBook)) {
        iv = vm.FormatInput(vm.tax.spsBook, 0, vm.LimD_BOOK)
        a = vm.FormatInput(vm.tax.spsBook, 0, 99999999999)
        v1 = '0'
        if (vm.YrEnd < 2023 && iv !== '0') {
          MsgID = 3
          b = 2023
        ...
        vm.tax.spsBook = SetTxt(vm.tax.spsBook, v1)
      }
      ...
      if (X > 0) {
        ...
        // 2023mock
        else if (X === 17) NullZero(vm.tax.slfBook, MsgID)
        else if (X === 18) NullZero(vm.tax.spsBook, MsgID)
    ...
    STCOut2_func () {
      ...
      // 2023mock
      vm.tax.slfBook = isNaN(parseFloat(vm.tax.slfBook)) ? 0 : parseFloat(vm.tax.slfBook)
      vm.tax.spsBook = isNaN(parseFloat(vm.tax.spsBook)) ? 0 : parseFloat(vm.tax.spsBook)
      ...
      // 2023mock
      if (vm.tax.slfOE + vm.tax.slfSEE + vm.tax.slfBook > this.STCIn2) {
        vm.tax.slfBook = this.STCIn2 - vm.tax.slfSEE - vm.tax.slfOE
      }
      if (vm.tax.spsOE + vm.tax.spsSEE + vm.tax.spsBook > this.STCIn3) {
        vm.tax.spsBook = this.STCIn3 - vm.tax.spsSEE - vm.tax.spsOE
      }
      ...
      this.jntBook = parseFloat(vm.tax.slfBook) + parseFloat(vm.tax.spsBook) // 2023mock
      this.STCIn16 = this.STCIn16 + parseFloat(vm.tax.slfERCE) + parseFloat(vm.tax.slfMpf) + parseFloat(vm.tax.spsERCE) + parseFloat(vm.tax.spsMpf) + parseFloat(vm.jntOE) + parseFloat(vm.jntSEE) + parseFloat(vm.jntRent) + parseFloat(vm.jntBook)
      if (this.STCIn2 > 0) {
      ...
      this.STCIn16 = this.STCIn16 + parseFloat(vm.tax.slfMedInsu) + parseFloat(vm.tax.spsMedInsu) + parseFloat(vm.tax.slfBook) + parseFloat(vm.tax.spsBook)
